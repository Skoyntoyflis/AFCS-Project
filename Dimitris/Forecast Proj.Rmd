...
tail(merged_data,1000)
```

Prepare sales dataset in a mergeable format.
```{r}
...
Merging all 3 datasets, to have the information on event, price and sells together. Should be easier to study correlations like this. (Hypothesis Testing?)

Notice how there are gaps in the item_id. This is because in the sell_prices doc the price is only provided for every week the product is available, otherwise there are gaps/missing weeks.
```{r}
result <- merged_data %>%
  mutate(date = as.Date(date, format = "%m/%d/%Y")) %>%
  left_join(sales_train_validation_long, by = c("date", "item_id"))
result
```
