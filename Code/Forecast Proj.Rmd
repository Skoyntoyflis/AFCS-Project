...
random_items <- sample(unique(merged_data$item_id), 10)

prices_selected <- merged_data %>%
...
plot <- prices_selected %>%
  plot_ly(x = ~date, y = ~sell_price, color = ~item_id, type = 'scatter', mode = 'lines') %>%
  layout(title = "Sales Over Time for 50 random items",
         xaxis = list(title = "Date"),
...
Regarding prices, we first plot some general statistics about the data. On plot 1 a) we can see that most of the items do not have a price during large periods in the dataset. This is possibly because not all food items are available throughout the year. In plot 1b),though, we can see that in the 5 years time period not a lot of price changes occur, with normalised price variations peaking at about 10% (plot 1d)), and a mean item price of 2.72$.
...
```{r}
jpeg(file="../Figures/average_prices.png",width = 500,height = 500)
sell_prices %>% 
  ...
  labs(x = "Average Sales Price [$]", y = "", 
       title = "Average Item Prices")

dev.off()

```
```{r}
jpeg(file="../Figures/number_price_changes.png",width = 700,height = 700)
foo <- stat_prices %>% 
  distinct(id, sell_price) %>% 
  group_by(id) %>% 
  summarise(mean_price = mean(sell_price),
...
#  geom_jitter(height = 0.1) +
  ...
  theme_hc() +
  theme(legend.position = "bottom") +
  labs( fill = "", title = "Number of Price changes")
...
# dev.off()
```
In order to undestand how prices influence sales, it is essential to see if price changes temporarily or for longer periods affect customer's buying habits.  The overall correlation between price changes and sales is $-0.1306$, indicating that when prices change, then a small decrease in sales is apparent, due to mostly price increases. In figure \ref{}, the sales for three example items are plotted, with the orange line representing the scaled prices. For some products, like "FOODS_3_030_TX_3" in 2013-2014, price changes infuence the sales significantly, while for others, like "FOODS_3_520_TX_3", consumers seem to buy in a steady rate, regardless of the price. Finally, it is noticeable that when there is a gap in sales for a long period, and then the price changes, most of the time sales pick up too.
...
merged_full_selected <- merged_data |>
  filter(item_id %in% random_items) %>%
  as_tsibble(key = item_id,
             index = date)

fit <- merged_full_selected |>
  model(base = ARIMA(sales, stepwise = FALSE, approx = FALSE))
```
...
```{r}
fc |>
  filter(item_id == "FOODS_3_416")|>
  autoplot(
    merged_full_selected,
    level = NULL
  ) +
  labs(y = "Sales" ) 
```
...
fit <- merged_full_selected |>
  model(
    `K = 1` = ARIMA(sales ~ fourier(K = 1)+ PDQ(0,0,0)),
    `K = 2` = ARIMA(sales ~ fourier(K = 2)+ PDQ(0,0,0)),
    `K = 3` = ARIMA(sales ~ fourier(K = 3)+ PDQ(0,0,0))#,
    # `K = 4` = ARIMA(sales ~ fourier(K = 4)+ PDQ(0,0,0)),
    # `K = 5` = ARIMA(sales ~ fourier(K = 5)+ PDQ(0,0,0)),
    # `K = 6` = ARIMA(sales ~ fourier(K = 6)+ PDQ(0,0,0))
  )
```
...
fit |>
  forecast(h = "28 days") |>
  autoplot(merged_full_selected, level = 95) +
...
```{r}
max_date_per_item <- sales_train_validation_long %>%
  group_by(item_id) %>%
  summarise(max_date = max(date))
print(max_date_per_item)

days_per_item <- sales_train_validation_long %>%
  group_by(item_id) %>%
